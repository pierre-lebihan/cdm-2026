...
function getCompetitionName(name: string | null | undefined): string {
...
function updateSeoMetadata(seoState: SeoState): void {
  ...
  ensureMetaByProperty('og:locale').content = 'fr_FR'
...
function SeoMetadata() {
  ...
  useEffect(() => {
    ...
    const competitionName = getCompetitionName(competition?.name)
    const seoState = getRouteSeo(location.pathname, competitionName)
    updateSeoMetadata(seoState)
  }, [competition?.name, loading, location.pathname])
